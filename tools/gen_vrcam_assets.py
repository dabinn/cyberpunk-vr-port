...
def make_component(w, h, cruid, parent_transform, name_prefix="vrcam_"):
    ...
    Player components receive the camera-slot binding they historically used. The dedicated world
    entity passes None, leaving the component directly on the entity root so SetWorldTransform()
    owns its complete pose and no player slot can move it afterwards.
    """
    component = {
...
def make_world_entity(wanted):
    """A component-only entity template for the world-owned VRCAM path.

    There is deliberately no mesh, animation, collider, physics controller, slot binding or
    gameplay record. DynamicEntitySystem supplies the entity's world transform at spawn time, and
    the runtime selector keeps that transform aligned to the authoritative MAIN camera snapshot.
    """
    entity = {
        "$type": "entEntity",
