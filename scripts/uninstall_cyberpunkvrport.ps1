...
# Grip-pose files are generated and packaged by prefix. Enumerate only immediate files in bin\x64;
# this keeps newly captured weapon poses uninstallable without walking any shared directory tree.
$binX64 = Resolve-RemovalTarget "bin\x64"
if (Test-Path -LiteralPath $binX64 -PathType Container) {
    Assert-NoReparsePointInPath $binX64
    foreach ($pose in (Get-ChildItem -LiteralPath $binX64 -File -Filter "CyberpunkVR_*.ini")) {
        Remove-OwnedFile $pose.FullName.Substring($RootPrefix.Length)
    }
}
...
foreach ($relative in @(
    ...
    "archive\pc\mod\vrport_basketball.archive",
