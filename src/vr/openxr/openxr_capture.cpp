...
extern "C" __declspec(dllexport) float CyberpunkVR_HudDistanceM = 1.8f;
...
bool OpenXRManager::EnsureVrcamEyeTexture(uint32_t width, uint32_t height, DXGI_FORMAT format) {
    ...
    if (m_vrcamEyeTex && m_vrcamEyeW == width && m_vrcamEyeH == height &&
        m_vrcamEyeFmt == static_cast<uint32_t>(format)) {
        return true;
    }
    if (m_vrcamEyeTex) { m_vrcamEyeTex->Release(); m_vrcamEyeTex = nullptr; }
    m_vrcamEyeSerial = 0;
    ...
    if (FAILED(m_d3dDevice->CreateCommittedResource(
            ...
            IID_PPV_ARGS(&m_vrcamEyeTex)))) {
        m_vrcamEyeTex = nullptr;
        Log("OpenXRManager: Failed to create stereo capture texture eye=1 %ux%u fmt=%u\n",
    ...
    m_vrcamEyeH = height;
    m_vrcamEyeFmt = static_cast<uint32_t>(format);
    SetD3DName(m_vrcamEyeTex, L"OpenXR_vrcam_eye");
    Log("OpenXRManager: stereo capture texture ready. eye=1 %ux%u fmt=%u\n",
        width, height, static_cast<unsigned>(format));
    return true;
}
...
bool OpenXRManager::CaptureMonoPresentedFrame(ID3D12Resource* backBuffer, const D3D12_RESOURCE_DESC& sourceDesc, uint64_t serial,
    ...
    if (m_captureFenceValue >= 3 && m_captureFence->GetCompletedValue() < m_captureFenceValue - 2) {
        ...
        if (WaitForSingleObject(m_captureFenceEvent, 100) != WAIT_OBJECT_0) {
            snapshot->Release();
            return false;
        }
    }
    ...
    // encode. What lands in m_vrcamEyeTex is then byte-identical in kind to MAIN's snapshot,
        ...
        if (vrcamSrc) {
            if (EnsureVrcamEyeTexture(eyeW, eyeH, DXGI_FORMAT_R8G8B8A8_TYPELESS)) {
                ...
                if (m_colorBlit->EnsureInitialized(m_d3dDevice,
                    ...
                    toRt.Transition.pResource = m_vrcamEyeTex;   // ours
                    ...
                    if (vrcamEyeCaptured && CyberpunkVR_BarrelDotSecondEye &&
                        ...
                        if (m_colorBlit->RecordDot(m_captureCmdList, m_vrcamEyeTex,
