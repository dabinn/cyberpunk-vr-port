...
// The follow loop and its base-fold bridge (src/Hooks/BodyYawFollow.cpp).
...
extern "C" void __fastcall OnOnFootDeltaHeadCallback(float* deltaHead) {
    ...
    // Physical body rotation (F10 -> Controls). OFF: no continuous body-yaw
    ...
    // bodyRot OFF -> classic snap-turn only: the heading never tracks the
